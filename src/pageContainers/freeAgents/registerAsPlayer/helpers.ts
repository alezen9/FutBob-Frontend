...
export const createPlayer = ({ setIsLoading, openSnackbar, setPlayerID }) => async (
	values: { _id, user: CreateUserInput & { country: OptionType }, player: Omit<CreatePlayerInput, 'user'> },
   ...
   try {
      const { _id, user, player } = values
      userID = await apiInstance.user.create({ ...user, country: user.country.value })
      if(!userID) throw new Error()
      playerID = await apiInstance.freeAgent.registerAsPlayer(_id, { ...player, user: userID })
