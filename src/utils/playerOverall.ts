...
export const getPlayerOverall = (score: PlayerScore, positions: PlayerPosition[]): number => {
   const zoneOverall = reduce(score, (acc, val, key) => {
   ...
   const zoneRoleImportance = [...new Set<Zone>(reduce(positions, (acc, val) => {
   ...
   const overall = reduce(zoneRoleImportance, (acc, val, idx) => {
